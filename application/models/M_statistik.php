    ...
    class M_statistik extends CI_Model
    {
        ...
        function insert_count()
        {
            if ($this->is_exist_count_now() == true) {
                $sql = "UPDATE tb_statistic SET statistic_visitor = (statistic_visitor+1) WHERE statistic_date = DATE(NOW())";
                return $this->db->query($sql);
            } else {
                $sql = "INSERT INTO tb_statistic (statistic_date, statistic_visitor) VALUES (NOW(), '1');";
                return $this->db->query($sql);
            }
        }
        ...
        {
            $sql = "SELECT statistic_id FROM tb_statistic WHERE statistic_date = DATE(NOW());";
            $query = $this->db->query($sql);
            if ($query->num_rows() > 0) {
                return true;
            } else {
        ...
        function delete($id = null)
        {
            ...
            $outp = $this->db->delete('tb_statistic');
